...
function Visuals() {
   ...
   const myState = useSelector((state) => state.updateProps);   // my
   //console.log(myState);
   ...
   const handlePlayPause = (play) => {   // true
        
      if(!myState.play){                 // !false = true
      ...
      dispatch({
         type: 'PLAY_PAUSE',
         _play: play    // visualization is going on set play to true 
      })
   }
            ...
            <div className="visual__items" style={{width:`${myState.values.length*11}px`}}> 
               {
                  myState.values.map((item) => {
                     ...
                     return <div className="visual__item" key={item[1]} id={item[1]} style={{transition:`${speed/1000}s linear all`, transform:`translateX(${item[1]*11}px)`}}>
                              <h4>{item[0]}</h4>
                              <div className="visual" style={{height:`${item[0]*3}px`, backgroundColor:color, width:(range<35? '9px': '6px')}}></div>    
                           </div>
                  })
               }   
            </div>
